...
### 캡션 쓰는 법
`<b>이름.</b> 명사로 끝나는 짧은 설명.` 한 줄로 끝낸다.

- **종결어미를 쓰지 않는다.** `…입니다`, `…합니다`, `…한다` 로 끝내지 않고 명사로 닫는다.
  캡션은 문장이 아니라 그림에 붙이는 이름표다.
- 제목에 이미 있는 말을 되풀이하지 않는다. 그림에서만 알 수 있는 것을 적는다.
- **출처는 적지 않는다.** 항목 끝 `src` 줄의 원문 링크가 그 일을 한다.

```
좋음  <b>H3 TAE.</b> 전체 디코드 전에 영상 latent을 훑는 소형 오토인코더.
나쁨  <b>H3 TAE.</b> 생성 중 영상 latent을 빠르게 미리 볼 수 있습니다.
```
